...
void jogar(int jogador){
	...
	do{
	printf("\nJogador %d -Insira a linha e a coluna :", jogador);
	scanf("%d %d", &lin, &col);
	lin--;
	col--;
	if (jogo[lin][col]!=' '){
...
int verificarJogo(){
	for (int i =0; i<3;i++){
		if (jogo[i][0]==jogo[i][1] && jogo[i][0]==jogo[i][2] && jogo[i][2]!=' ')
			return 1;
		if (jogo[0][i]==jogo[1][i] && jogo[0][i]==jogo[2][i] && jogo[2][i]!=' ')
			return 1; 
		}
			
		if (jogo[0][0]==jogo[1][1] && jogo[0][0]==jogo[2][2] && jogo[2][2]!=' ')
			return 1;
		if (jogo[0][2]==jogo[1][1] && jogo[0][2]==jogo[2][0] && jogo[1][1]!=' ')
			return 1;
...
void parabens(int jogador, int vencedor){
	if (vencedor)
	switch (jogador){
		case 1: printf("\n Parabén o Jogador 1, venceu");
				break;
		case 2: printf("\n Parabén o Jogador 2, venceu");
				break;
	}
	else
		printf("Empate!!!!");
...
int main(void)
{
	int vencedor=0, jogadas=0, jogador=0 ;
	...
	while(vencedor==0 && jogadas<9){
		...
		mostrarJogo();
		jogar(jogador);
		vencedor =verificarJogo();
		
		jogadas++;
	}
	mostrarJogo();
	parabens(jogador, vencedor);
